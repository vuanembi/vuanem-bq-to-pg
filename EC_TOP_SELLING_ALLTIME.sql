 ...
 SELECT alltime."PRODUCT_CODE",
   ...
   FROM ( SELECT items."PRODUCT_CODE",
          ...
          WHERE transactions."TRANSACTION_TYPE"::text = 'Sales Order'::text AND transactions."STATUS"::text <> 'Closed'::text AND transactions."TRANSACTION_NUMBER" IS NOT NULL AND items."DISPLAYNAME" IS NOT NULL AND (transaction_lines."ACCOUNT_ID" IN ( SELECT "ACCOUNTS"."ACCOUNT_ID"
